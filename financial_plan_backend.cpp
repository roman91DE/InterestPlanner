...
financial_plan::financial_plan(double _initial_debt, double _interest_rate,
                               int _total_time)
    : initial_debt(_initial_debt), interest_rate(_interest_rate),
...
std::vector<std::vector<double>> financial_plan::get_matrix() { return plan; }

int financial_plan::get_totalTime() const { return this->total_time; }

// konstante tilgung
constant_repayment::constant_repayment(double _initial_debt,
                                       double _interest_rate, int _total_time)
...
void constant_repayment::compute() {
  ...
  for (unsigned int i = 0; i < total_time; i++) {
    ...
    plan[i].push_back(
        financial_plan::calculate_interest(interest_rate, remainder));
...
// konstante annuität
constant_annuity::constant_annuity(double _initial_debt, double _interest_rate,
                                   int _total_time)
    : financial_plan(_initial_debt, _interest_rate, _total_time) {
  compute();
}
...
double constant_annuity::get_annuity() {
  double z = (interest_rate / 100) *
             std::pow((1 + (interest_rate / 100)), (total_time));
...
// konstante tilgung
constant_interest::constant_interest(double _initial_debt,
                                     double _interest_rate, int _total_time)
    : financial_plan(_initial_debt, _interest_rate, _total_time) {
  compute();
}
...
void constant_interest::compute() {
  double interest =
      financial_plan::calculate_interest(interest_rate, initial_debt);
  for (unsigned int i = 0; i < total_time; i++) {
    std::vector<double> cur_year;
    plan.push_back(cur_year);
    plan[i].push_back(float(i) + 1);
