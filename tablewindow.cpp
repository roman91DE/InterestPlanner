    ...
    : QTableWidget(parent), finance_plan(f)
{
    ...
    const QStringList header_horizontal = { "Periode", "Tilgung", "Zinszahlungen", "Annuität", "Restliche Schuld" };
    this->setHorizontalHeaderLabels(header_horizontal);
    std::vector < std::vector <double>> data = finance_plan->get_matrix();
    for (int i = 0; i < rows; i++) {
        for (int k = 0; k < cols; k++) {
            QTableWidgetItem *temp = new QTableWidgetItem(QString::number(data[i][k]));
